...
var gameIsRunning = true;  // Запущена ли игра
var snake_timer;            // Таймер змейки
var food_timer;             // Таймер для еды
var score = 0;              // Результат
var scoreTime;              // текущий счет
...
function init() {
    ...
    scoreTime = document.getElementById('score-time')[0];
    scoreTime = score;
	/*
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && !haveBlock(new_unit)) {
...
function createBlocks(){
    ...
    while(!createB){
        var blockX = Math.floor(Math.random()*20);
        var blockY = Math.floor(Math.random()*20);

        var block_cell = document.getElementsByClassName('cell-'+blockY+'-'+blockX)[0];
